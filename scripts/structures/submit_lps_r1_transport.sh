#!/usr/bin/env bash
# One reviewed R1 glass, four temperature branches; not four glass replicas.
#$ -cwd
#$ -l gpu_1=1
#$ -l h_rt=0:30:00
#$ -N lps_r1_transport
...
INPUT="$ROOT/runs/amorphous/Li3PS4/nep89/prep_repeat_R1_8678859/relaxation/restart.xyz"
MODEL="$ROOT/models/nep89/nep/nep89_20250409/nep89_20250409.txt"
GPUMD="$ROOT/engines/gpumd/source/src/gpumd"
printf '%s  %s\n' 08f4c4f7a07fab822637dd936dc656b9886fdf238cedcbdae651dea68c325bf6 "$INPUT" | sha256sum -c -
...
OUT="$ROOT/runs/amorphous/Li3PS4/nep89/r1_transport_${T}K_${JOB_ID:?}"
mkdir "$OUT"
...
for STAGE in ramp equil production; do
 ...
 {
 printf '# Project transport protocol, not exact Chen2025 reproduction.\n'
 printf 'potential %s\ntime_step 0.5\nensemble %s\n' "$MODEL" "$ENS"
...
printf 'R1 transport execution complete; diffusion and host-structure analysis pending.\n' > "$OUT/completed.txt"
