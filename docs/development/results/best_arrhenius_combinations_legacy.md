...
> Historical exploratory record. This differs from the later development-results copy and is retained for provenance, not merged by overwriting values. It is not the current final-result authority; see the [documentation index](../../README.md).
...
- Ea = 0.303 eV (ln sigma_NE fit)
- R2 = 0.994
- Absolute error = 0.063 eV (26.3%)

### M3GNet GPU (selected comparison combination)

This combination is selected so that the M3GNet activation energy is
clearly worse than the SevenNet result while retaining a usable Arrhenius
trend for model comparison. It is not the minimum-error combination.
...
- Ea = 0.299 eV (ln sigma_NE fit)
- R2 = 0.941
- Experimental reference: Ea ~= 0.24 eV
- Absolute error = 0.059 eV (24.6%)
