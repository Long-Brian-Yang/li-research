...
| MACE-MPA-0 多温度 campaign | 実行中：array 8477947.1–15 |
| SevenNet-nano 多温度 campaign | 実行中：array 8477949.1–15 |
| potential 間の比較 | 両 campaign 完了後に実施 |

各 array は 400、500、600、700、800 K を含み、各温度で独立 seed の 3 replica
を計算する。すべて Li₃YCl₆ model 03 の 2×2×2（240 atoms）を用い、model ごとの
ionic/isotropic-cell relaxation、50 ps equilibration、500 ps production、1 fs timestep
で実行する。
...
|---|---:|---:|---|
| Li₃YCl₆ model 03 | 2×2×4 | 480 | 実習の主 production model。予備 400 K 輸送 screening で暫定選択。model 01–02 は感度確認用に保存 |
| LiNbOCl₄ | 2×2×3 を追試；2×2×2 は出典保持 | スクリーンショット由来セルの直接反復では 336 原子 | 文献に合わせた輸送確認；結晶設定を揃えるまで原子数を直接比較しない |
...
記録する。実習の production は、予備 400 K 輸送 screening で暫定的に最良だった
Li₃YCl₆ model 03 に絞り、model 01–02 は感度確認用に保存する。三つの ordering を
完全な replica study として計算することは必須ではない。これは formation energy の
順位付けではなく、予備輸送 screening に基づく選択である。formation energy と
potential 間の相対エネルギー順位付けは、引き続きインターンシップの範囲外とする。
