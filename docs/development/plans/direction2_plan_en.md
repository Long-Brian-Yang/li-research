...
| MACE-MPA-0 multi-temperature campaign | Running: array 8477947.1–15 |
| SevenNet-nano multi-temperature campaign | Running: array 8477949.1–15 |
| Matched potential comparison | After both campaigns finish |

Each array covers 400, 500, 600, 700 and 800 K with three independent seeds at
each temperature. Every task uses Li₃YCl₆ model 03 in a 2×2×2 (240-atom) cell,
model-specific ionic/isotropic-cell relaxation, 50 ps equilibration, 500 ps
production and a 1 fs timestep.
...
|---|---:|---:|---|
| Li₃YCl₆ model 03 | 2×2×4 | 480 | Main internship production model; provisionally selected from the preliminary 400 K transport screen. Models 01–02 remain archived sensitivity controls. |
| LiNbOCl₄ | 2×2×3 follow-up; 2×2×2 retained for provenance | 336 atoms in the direct screenshot-cell repeat | Literature-aligned transport check; compare atom counts only after matching crystallographic settings |
...
cell shape was fixed must be recorded. For the internship production run, use
Li₃YCl₆ model 03, provisionally selected from the preliminary 400 K transport
screen; archive models 01–02 as sensitivity controls. A full three-ordering
replica study is optional. This is not a formation-energy ranking; formation
energy and cross-potential relative-energy ranking remain outside the internship
scope.
