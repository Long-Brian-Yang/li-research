...
The eight model/interface entries below are complete as a **speed benchmark**.
They use one common relaxed 240-atom Li₃YCl₆ geometry,
`Li3YCl6_03_mace_cell_relaxed.data`, converted to the native format required by
each backend. This makes interface-speed comparisons possible, but it is not a
physical diffusion comparison: production transport calculations require
model-specific relaxation, stability checks, and independent MD convergence.

For each valid entry, the benchmark record must retain its input, model
identity/checksum, executable, stdout/stderr, and LAMMPS/GPUMD performance line
in the corresponding timestamped TSUBAME result directory.
