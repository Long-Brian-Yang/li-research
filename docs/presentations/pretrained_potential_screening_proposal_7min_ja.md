...
ページ数は16枚とするが、各ページの説明を15–35秒に限定し、発表時間は合計7分に維持する。高イオン伝導材料については、一般に高い拡散係数 `D`、高い伝導度 `σ`、低い活性化エネルギー `Eₐ` を望ましい方向とする。
...
|Page|内容|時間|累積|
|---:|---|---:|---:|
|1|NGK材料探索と本研究の位置づけ|0:15|0:15|
|2|初期CPU–GPU・モデルベンチマーク|0:25|0:40|
|3|二つの結晶材料ワークフロー|0:25|1:05|
|4|結晶材料のMSD結果|0:25|1:30|
|5|結晶材料の活性化エネルギー|0:25|1:55|
|6|結晶から非晶質への移行|0:15|2:10|
|7|MACEとNEP89の比較・選定|0:30|2:40|
|8|試行した非晶質材料群|0:20|3:00|
|9|材料ごとに異なる非晶質ワークフロー|0:30|3:30|
|10|LZOC・LSZCの結果|0:35|4:05|
|11|Li₃PS₄・LiPONの結果|0:30|4:35|
|12|技術的難点と得られた判断基準|0:25|5:00|
|13|提案1：材料探索の全体アーキテクチャ|0:30|5:30|
|14|提案2：計算とNGK実験の受け渡し|0:25|5:55|
|15|提案3：ポテンシャル選択とfine-tuning|0:30|6:25|
|16|提案4：90日導入計画とKPI|0:35|7:00|
...
# Page 13｜提案1：計算とNGK実験を循環させる全体アーキテクチャ
...
**レイアウト：** 上段70%を閉ループワークフロー、下段30%を多特性スコアカードにする。
...
「全体提案は、NEP89で広く探索し、Evidence GateでGreen、Yellow、Redに分ける構成です。GreenはNGK実験へ、Yellowは別モデル照合へ、Redは材料特化学習または保留へ進めます。実験データは次の候補生成と学習へ戻します。」

## 次ページへの接続

「このループを動かすために、計算側とNGK側の受け渡し内容を標準化します。」

---

# Page 14｜提案2：計算結果を実験判断へ変える標準受け渡し

**発表時間：5:30–5:55**

## 1ページの結論

候補名だけを渡すのではなく、構造、計算条件、輸送、局所構造、信頼度、確認したい実験項目を一つのEvidence Packageとして渡す。

**レイアウト：** 左45%を計算側パッケージ、中央10%を矢印、右45%をNGK評価とfeedbackにする。

## スライド表示：役割と受け渡し

```mermaid
flowchart LR
    subgraph UNI["計算側｜Composition Proposal Package"]
        A1["候補ID・組成・構造"]
        A2["作製履歴・MD条件・Potential"]
        A3["D(T)・Eₐ・σ・RDF・配位"]
        A4["信頼度・既知リスク・実験優先順位"]
    end
    A1 --> B["Evidence Package"]
    A2 --> B
    A3 --> B
    A4 --> B
    B --> C1
    subgraph NGK["NGK｜Synthesis & Validation"]
        C1["合成条件・相同定"] --> C2["伝導・成形・安定性"]
        C2 --> C3["成功／未達・観察事実"]
    end
    C3 --> D["標準化データ<br/>次周期へfeedback"]

    classDef calc fill:#dceeff,stroke:#1769aa,color:#0d3557,stroke-width:1.5px;
    classDef package fill:#fff2cc,stroke:#b8860b,color:#5c4700,stroke-width:2px;
    classDef exp fill:#e8f5e9,stroke:#2e7d32,color:#17451a,stroke-width:1.5px;
    classDef data fill:#eee8f8,stroke:#6d4ca3,color:#38235f,stroke-width:1.5px;
    class A1,A2,A3,A4 calc;
    class B package;
    class C1,C2,C3 exp;
    class D data;
```
...
|構造|計算|結果|判断|
|---|---|---|---|
|CIF／XYZ、密度、作製履歴|Potential、温度、時間、系統|`D(T)`、`Eₐ`、RDF、配位|信頼度、比較文献、推奨実験|

## 口頭講稿

「受け渡すのは候補組成だけではありません。構造、作製履歴、Potential、MD条件、輸送、局所構造、文献差、信頼度、推奨実験を一つのEvidence Packageにします。NGKの合成・評価結果も同じ候補IDへ戻し、次周期で再利用します。」

## 次ページへの接続

「次に、どの候補をNEP89だけで進め、どこで別モデルやfine-tuningを使うかを明確にします。」

---

# Page 15｜提案3：ポテンシャル選択とfine-tuningの開始条件

**発表時間：5:55–6:25**

## 1ページの結論

NEP89を全候補の入口とし、モデル不一致、構造異常、文献との桁差、事業重要度に応じて計算精度を段階的に上げる。

**レイアウト：** 上段60%を判断木、下段40%をGreen／Yellow／Redの運用表にする。

## スライド表示：ポテンシャルの段階的高度化

```mermaid
flowchart LR
    A["候補組成"] --> B["NEP89 zero-shot<br/>構造作製・多温度MD"]
    B --> C{"構造・輸送ゲート"}
    C -->|"通過"| D["MACE／SevenNet<br/>代表条件で照合"]
    C -->|"異常"| R["Red<br/>fine-tune／保留"]
    D --> E{"傾向・順位が一致?"}
    E -->|"Yes"| G["Green<br/>NGK実験へ"]
    E -->|"一部不一致"| Y["Yellow<br/>長時間化・追加温度・再照合"]
    Y --> E
...
|区分|条件|次の処理|
|---|---|---|
|Green|構造ゲート通過、モデル傾向一致、文献範囲内|NGK実験候補|
|Yellow|傾向は一致するが絶対値・samplingに不確実性|長時間化、追加温度、別モデル照合|
|Red|骨格異常、モデル順位逆転、文献と桁差|材料特化fine-tuningまたは保留|

## fine-tuning用データの優先範囲

`構造緩和 → melt–quench高エネルギー配置 → 平衡構造 → Li跳躍近傍 → 実験対象組成`

## 口頭講稿

「NEP89は全候補の入口にします。構造と輸送ゲートを通過した代表条件だけをMACE等で照合し、一致すれば実験へ進めます。不一致が限定的なら計算を追加し、骨格異常や桁違いの偏差があれば材料特化fine-tuningまたは保留とします。」

## 次ページへの接続

「最後に、この仕組みを小さく導入して評価する90日計画を示します。」

---

# Page 16｜提案4：90日パイロットと評価KPI

**発表時間：6:25–7:00**

## 1ページの結論

一つの重点化学系から開始し、標準化、候補選定、NGK評価feedbackまでを90日で一巡させ、次段階への投資判断を行う。

**レイアウト：** 上段55%を3段階ロードマップ、下段30%をKPI、最下段15%を今回求める意思決定にする。
...
gantt
    ...
    axisFormat  %m/%d
    section Phase 1
    入力・MD・Evidence Package標準化 :a1, 2026-10-01, 28d
    section Phase 2
    候補計算・モデル照合・優先順位 :a2, after a1, 28d
    section Phase 3
    NGK評価feedback・MI／fine-tune判定 :a3, after a2, 28d
```

> 実施日は承認時点に合わせて置き換える。重要なのは4週間×3段階の順序である。

## 評価KPI

|計算効率|科学的品質|実験接続|事業判断|
|---|---|---|---|
|候補／GPU日、候補あたりコスト|品質ゲート通過率、モデル一致率|合成着手率、実験との傾向一致|次段階候補数、追加学習の必要性|

## 今回求める意思決定

1. パイロット対象とする重点化学系を一つ選ぶ。
2. 計算側とNGK側で共有するEvidence Package項目を合意する。
3. 90日後にKPIで継続・拡張・fine-tuningを判断する。

## 口頭講稿

「導入は一つの重点化学系から始めます。最初の4週間で入力、MD、Evidence Packageを標準化し、次の4週間で候補計算とモデル照合、最後の4週間でNGK評価結果を戻してMIまたはfine-tuningの必要性を判断します。今回合意したいのは、対象化学系、共有データ項目、90日後のKPIです。」

## 最後の一文

**ベンチマーク、結晶検証、非晶質適用範囲の結果を、候補提案から実験フィードバックまで追跡可能なNGK材料探索基盤へ変換する。**
...
- 16ページを順番に表示するが、各ページは表中の時間で切り替える
