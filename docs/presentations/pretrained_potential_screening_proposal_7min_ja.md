# 汎用原子間ポテンシャルを用いたNGK材料探索workflowの高度化

## 7分間・企業向け発表原稿

本資料は、研究の実施順序に沿って、**計算基盤のbenchmark → 結晶材料でのモデル検証 → 非晶質計算への拡張 → 文献・実験との照合 → NGK材料探索workflowへの実装提案**を7ページで説明する。各ページに表示内容、図表、口頭講稿、次ページへの接続をまとめる。

高イオン伝導材料の探索では、一般に**高い拡散係数 `D`、高い伝導度 `σ`、低い活性化エネルギー `Eₐ`**を望ましい方向とする。

## NGK材料グループとの接続

本提案は、固体電解質をイオン伝導度だけで選ばず、成形性、コスト、耐酸化性、化学安定性、耐熱性、合成可能性を含めて評価する材料探索を想定する。今回のMDは主に**Li輸送、有限温度構造、耐熱挙動**を担当し、他の特性は組成記述子、第一原理計算、既存実験データ、NGKでの合成・評価を組み合わせて補完する。

```mermaid
flowchart LR
    A[計算側<br/>候補組成・構造・輸送予測] --> B[NGK<br/>合成・特性評価]
    B --> C[実験データ蓄積]
    C --> D[MI／ポテンシャル更新]
    D --> A
```

> 公開版では、社内管理対象の具体的な目標値、候補組成、実験データ表は掲載しない。実際の社内発表では、管理された仕様表から対応値を挿入する。
...
**NGKとの接続：** 既存のM3GNet／MDによる計算的材料スクリーニングを、高速化とモデル比較によって拡張する段階に相当する。

既存の結晶候補スクリーニングを置き換えるのではなく、GPU化、複数ポテンシャル照合、非晶質・長時間MDを追加する。
...
**NGKとの接続：** 候補組成を提案する前に、既知の固体電解質で計算手法の再現性とモデル依存性を確認する基準段階である。
...
**NGKとの接続：** 実験候補を増やすための計算スループットを確保し、組成提案前の予備選別を短時間化する。
...
- 候補組成と初期構造
- Li輸送指標：`D(T)`、`Eₐ`、条件付き `σ`
- 有限温度での構造安定性、RDF、配位情報
- 推奨する合成・評価優先順位と、予測信頼度
...
**NGKとの接続：** 計算値だけで完結せず、実験伝導度と局所構造を同時に照合してから候補を判断する実例である。
...
**NGKとの接続：** 一つの材料群に限定せず、塩化物、オキシハライド、硫化物、酸窒化物を共通workflowで比較し、実験へ渡す候補群を広く探索する。
...
# Page 7｜NGK向け提案：多特性スクリーニングと実験feedback

**発表時間：5:50–7:00**

## このページの結論

全候補を高精度化するのではなく、NEP89で広く探索し、複数特性と予測信頼度で候補を絞り、NGKの合成・評価結果を次のMI／fine-tuningへ戻す。

## 提案する運用

```mermaid
flowchart LR
    A[Stage 1<br/>計算で候補組成を提案] --> B[Stage 2<br/>多特性・信頼度で選別]
    B --> C[Stage 3<br/>NGKで合成・評価]
    C --> D[実験データ]
    D --> E[MI／fine-tuning]
    E --> A
```
...
|段階|実施内容|NGK材料探索への出力|
|---|---|---|
|Stage 1：高速探索|構造作製、多温度MD、組成記述子、公開データを統合|候補組成と計算根拠|
|Stage 2：多特性選別|輸送、成形性、コスト、電気化学・化学・熱安定性、合成可能性を評価|実験優先順位と信頼度|
|Stage 3：実証と更新|NGKで合成・評価し、得られた結果をMI／ポテンシャルへ戻す|次周期の改善候補|
...
「NGK向けには、計算と実験を循環させる三段階の導入を提案します。まずNEP89を中心に候補組成とLi輸送を高速評価します。次に、伝導だけでなく、成形性、コスト、電気化学・化学・熱安定性、合成可能性をscorecardで確認し、実験優先順位を付けます。NGKで合成・評価した結果は、次のMI探索と必要な化学系のfine-tuningへ戻します。この運用により、高伝導だけでなく、実用に必要な特性を兼ね備えた候補へ探索を収束できます。」

## 最後の一文

**CPU–GPU benchmark、結晶基準、非晶質検証で得た計算基盤を、NGKの合成・評価と循環させることで、多特性を兼ね備えた固体電解質候補をより速く提案できる。**
...
|成形性|弾性率・体積応答の計算モジュールを追加|成形試験、密度、機械特性|
|低コスト|希少元素制約、原料情報、組成フィルタ|調達性・プロセスコスト|
|耐酸化・化学安定性|第一原理の反応・電位安定性を追加|電気化学窓、界面・保存試験|
|耐熱性|有限温度MDによる骨格・密度変化|加熱後の構造・特性評価|
|合成可能性|相安定性、既知構造、類似組成から信頼度評価|実際の合成と相同定|
...
|期間|実施内容|成果物|
|---|---|---|
|1–4週|入力、MD条件、多特性scorecard、NGKへの出力形式を標準化|再利用可能なworkflow|
|5–8週|候補群を計算し、重点候補を別モデル・文献で照合|候補組成、根拠、実験優先順位|
|9–12週|NGK評価結果を取り込み、MI／fine-tuningを小規模実証|次周期での候補改善量|
