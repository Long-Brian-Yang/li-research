...
### 文献比較の優先順位：2026年9月15日確認

TSUBAME会計記録でLSZCの8676216.1–4は全てfailed=0、exit_status=0（最後の終了14:31:35 JST）。各温度に300 ps完了マーカーとproduction熱力学記録6000行がある。上記の投入状態を更新し、現在はproduction完了・解析待ちとする。今回の確認で新規ジョブは投入していない。

|優先順位・材料|文献比較に必要な作業|今すぐ追加MDが必要か|
|---|---|---|
|1. LSZC|完了済み320／330／340／350 KからMSD、D(T)、条件付き伝導率、根拠のあるE_aを解析する。Tangの温度別数値、次に303.15 K実験値と比較し、密度・配位差、既存Li–O／移動の相関を解釈に含める。|不要。既存軌跡で必要な輸送推定ができない場合のみ延長を検討する。|
|2. 新LZOC|完了済み2 fs系列をHussainのtracer D*と比較する。現行図表に残る0.5 fs対照は主表示から整理する必要がある。Li／Cl運動と局所構造を比較し、局所振動と長距離拡散、tracer D*とcharge Dを区別する。|追加刻み幅比較は不要。作製・原子数・ポテンシャルの違いは明記する。|
|3. Li₃PS₄|既存輸送・構造差をChenのDeePMDガラス結果と関連付け、参照をAIMD／実験とはしない。300 KのMSD停滞は伝導率の主張を制限し、293.15 K実験値とは同温度ではない。|直ちに不要。室温の定量値を主張する場合のみ長時間化を検討する。|
|4. LiPON|短いN–N接触と参照構造との差を予備学習ポテンシャルの限界として説明し、再現成功とはしない。|盲目的延長・DFTは不要。未解決構造から輸送性能を主張しない。|
|5. 旧LZOC|密度緩和の議論を残す場合、完了済み追加NPT軌跡を解析する。速度・高温比較は補足的位置付けとする。|追加計算不要。|

全PDF／構造因子比較は任意で、参照数値と散乱の定義が必要だが原則として新productionは要しない。独立ガラス統計も単一構造の探索比較の必須条件とはせず、未評価という限界を残す。数値差だけを理由に再計算や軌跡の選び直しをしない。
