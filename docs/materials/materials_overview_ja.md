...
Sethらは13,454個のDFT構造からLiPON専用NequIPを学習し、非晶質Li2.94PO3.5N0.31を作製した。主な構造的結果は、Nがapicalとbridgingの両位置に存在し、PO4、PO3Nおよび縮合リン酸塩ユニットが共存することである。その上で300–900 KのバルクLi拡散と伝導度を評価している。[Sethら、2025；DOI: 10.1021/acsmaterialsau.4c00117](https://doi.org/10.1021/acsmaterialsau.4c00117)
...
圧力と密度はセル平均であり、局所化学を保証しない。一方、短距離のみで特定反応も確定できない。3組比較が支持する限定的な解釈は、NEP89が酸素骨格を再現的に保つ一方、窒素トポロジーは溶融軌跡に敏感であることである。

**LiPONの結論：** Preparation Bは、拡散結果を見る前に定めた構造確認を通過したため、探索的バルク輸送系列の作業構造とする。AとCは削除せず、適用性の失敗例として報告する。次はSethの300–900 Kに合わせ、完全Li MSD、D(T)、骨格MSD、Arrhenius挙動を示す。production完了までの本節の結論は構造的準備性であり、伝導度の再現ではない。

<a id="remaining"></a>
## 6. 総括：再現できた点・偏差・次の課題

結論は二つに分ける。実測した計算系ではNEPの計算コストが低く、採用の動機となった。一方、文献比較は一様な予測精度を示さない。LZOCはAIMDのトレーサー拡散を過小評価し、LSZCは温度傾向を再現せず、Li₃PS₄は局所構造の部分的一致に対して輸送の定量的一致を示さず、LiPONは熱履歴に敏感な窒素トポロジーを示す。実行時間だけでなく、これらの材料依存の結果が現時点の適用限界を示す。LSZC端点NPTは完了したが、350 Kの末段密度低下が残り、両温度の平衡確認には至らない。
...
|LiPON|3組を解析。Bは短いN–Nを持たず、apicalとbridging Nを保持|Bで1組の文献温度バルク輸送系列へ進む。DFTなし。|
...
完了範囲は予備学習ポテンシャルの文献対応比較であり、全材料の実験／AIMD再現成功ではない。LiPONの3組の作製軌跡は完了・解析済みであり、表で解析待ちとした輸送計算のみが未完である。物理的限界は結果として残し、都合のよい軌跡の選択で「修正」しない。
...
|LiPON|124：Li47P16O56N5|3組作製、除圧、RDF、配位、時間刻み対照を解析|窒素トポロジーは作製依存；Bは探索的輸送に使用可能|
|旧LZOC|192、同公称LZOC|600 K詳細比較、700–900 K MSD／RDF、四温度計時を解析済み|計算効率・構造感度の比較であり精度順位ではない|

最新LSZC端点のproductionと解析は完了した。Li₃PS₄輸送とLiPONの3組の作製軌跡は上記で解析済みである。DFTは投入していない。
