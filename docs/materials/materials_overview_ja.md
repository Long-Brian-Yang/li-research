...
512原子の追加作製を2組投入した。同じ前駆体から**溶融・冷却より前に速度を変更**し、完成済みガラスのproductionだけを繰り返す方法とは区別する。目的は作製依存性の検討であり、文献に最も近い数値の選別ではない。上記の図は完了済み結果であり、新規計算の解析結果はまだない。
...
|Li₃PS₄|既存の輸送・局所構造比較を保持|611 psの作製を2組投入。構造確認後にproductionを判断。|
