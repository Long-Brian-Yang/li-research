...
600 K productionの時間比は**31.1倍**である。四温度のジョブ全体時間の合計は、**MACEが18時間25分、NEP89が34分**である。ただし、並列経過時間やポテンシャル演算だけのベンチマークではなく、エンジン、ノード、前処理、速度初期化、密度、構造が異なる。
...
![LZOCのArrhenius座標におけるLiトレーサー拡散](figures/29_LZOC_Arrhenius_comparison.png)

同じ3点をArrhenius座標で示しても、NEP89とAIMDはいずれも単調な直線系列にならない。このためLZOCについては、同温度の $`D`$ 比較は成立する一方、正式な $`E_a`$ は報告しない。これは図の欠落ではなく、現在の温度点から一意な熱活性化障壁を定めないという整理である。
...
![LSZCの放射光PDFとZr平均配位数](figures/27_LSZC_PDF_and_Zr_coordination.png)

パネルaは実験放射光PDF、論文の調整済みMACE PDF、320 K NEP89軌跡から計算したX線加重代理量を並べる。公開実験と調整済みMACEは主要ピークを共通して再現する一方、NEP89代理量は第一配位殻の主特徴を保持するが、それ以遠で差を示す。代理量は一定の原子番号重みを用いて形状比較のため規格化しており、完全なQ依存放射光PDFとは称さない。パネルb、cは平均Zr配位数の温度依存性をEXAFS基準と直接比較する。320–350 KでNEP89のZr–Oは**1.61–1.68**、Zr–Clは**4.21–4.30**の狭い範囲にあり、温度による変化は小さい。一方、EXAFS平均の**O 2.6個、Cl 3.0個**に対して、全温度でO配位が少なくCl配位が多い。従って、硫酸根保持と輸送活性化を再現しながらも、Zr局所環境には実験ガラスとの差が残る。[Tangら、2026；DOI: 10.1038/s41467-026-69737-x](https://doi.org/10.1038/s41467-026-69737-x)
...
![Li3PS4のLi拡散Arrhenius解析](figures/31_Li3PS4_Arrhenius_comparison.png)

NEP89の4温度 $`D`$ を表示し、熱活性化障壁は直線性の高い500–900 Kから評価した。ChenのDeePMD系列には論文報告の0.470 eV傾きを重ねた。両者は同じ自己拡散量を比較しており、室温実験伝導度はこの $`\ln D`$ 軸へ混在させない。
...
|ルート|実施条件|参照と相違|
|---|---|---|
|LZOC|100 K、2 ps；500 K、30 ps；1,000 K、50 ps；1,500 K、30 ps；2,000 K、20 ps；1,500／1,000／500／100 Kを各2 ps経由して冷却；300 Kで20 + 50 ps緩和。|[Hussain 2024](https://doi.org/10.1038/s41524-024-01346-y)：192原子NEP再構築であり、48原子AIMD輸送モデルおよび完全な作製手順とは異なる。|
|LSZC|5種類の有限団簇を各2個と32 Liで構成；固定セル位置緩和は0.0493 eV/Åまで；300 K NVT、20 ps；400 Kまで100 ps昇温；20 ps保持；400 K、1 bar NPT、20 ps；NVT、200 ps。|[Tang 2026](https://doi.org/10.1038/s41467-026-69737-x)：独立し272原子装箱であり、著者の1,088原子構造および調整済みMACEとは異なる。|
|Li₃PS₄|1,500 K NPT、100 ps；1,500から300 Kまで480 psで冷却（2.5 K/ps）；300 Kで20 ps保持；各目標温度で10 ps昇温 + 1 bar NPT、50 ps + NVT、200 ps；時間刻み0.5 fs。|[Chen 2025](https://doi.org/10.1038/s41467-025-56322-x)：熱履歴を参照したが、DeePMDをNEPへ置換し、初期構造、結合時間、production条件は異なる。|
|LiPON|2,000 K、10 ps；2,000から250 Kまで7 psで冷却；250 Kで20 ps保持；250 K、1 barで20 ps除圧。Preparation B輸送は600／900／1,200／1,500 K、10 ps NPT昇温 + 50 ps NPT + 300 ps NVT；時間刻み0.5 fs。|[Seth 2025](https://doi.org/10.1021/acsmaterialsau.4c00117)：バルク温度は対応するが、NEP、独立に生成した前駆体、結合条件、計算時間はNequIP研究と異なる。|
...
|$`\mathbf r_i(t)`$|アンラップし、ドリフト補正した原子 $`i`$ のCartesian座標|Å；$`1\,\text{Å}=10^{-10}\,\mathrm{m}`$|
...
|$`V`$|瞬間または明記したセル体積|Å³；$`1\,\text{Å}^3=10^{-30}\,\mathrm{m^3}`$|
...
係数6は三次元拡散の $`2d`$（$`d=3`$）であり、意図的に一次元／二次元へ射影したMSDでは変更が必要である。単位換算は $`1\,\text{Å}^2/\mathrm{ps}=10^{-4}\,\mathrm{cm^2/s}`$。安定した長時間拡散領域を確認できない場合、回帰値を $`D_{\mathrm{app}}`$ と表記する。図示した全軌跡と回帰区間は同義ではなく、利用可能という理由だけで時間原点の少ない末尾を回帰に含めない。

### 伝導度とArrhenius量

$`V`$ はセル体積、$`e=1.602176634\times10^{-19}`$ CはLi⁺の素電荷、$`k_B=1.380649\times10^{-23}`$ J/K、$`T`$ は絶対温度である。SI計算では $`D[\mathrm{cm^2/s}]\times10^{-4}`$ でm²/s、$`V[\text{Å}^3]\times10^{-30}`$ でm³へ変換する。出力単位の関係は
