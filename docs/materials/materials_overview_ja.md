...
新規結果は未解析で、下記図は完了済みの二端点結果である。両方の四温度系列が終了後、0–300 ps MSDを比較し、拡散的挙動と傾き安定性に基づく共通の判定基準でDを求める。表示上は代表軌跡を用いる場合も、対応する系列と全2組の結果を元記録に保持し、E_aを良くするためだけに温度ごとの結果を混在させない。Arrhenius解析が支持される場合のみ固定密度のE_aを示し、実験0.33 eVとは条件を区別する（[Tangら、DOI: 10.1038/s41467-026-69737-x](https://doi.org/10.1038/s41467-026-69737-x)）。追加サンプリング自体は有効なE_aを保証しない。
...
4点のNEP89拡散係数から見かけのArrhenius活性化エネルギーは**0.412 eV**（R²=0.99975）となり、LiPON薄膜で一般に報告される実験値約0.55 eVより低い。しかし、勾配だけが妥当でも絶対値は文献と一致しない。600 KでSethのmelt-quench LiPONは1.25×10⁻¹⁰ cm²/sであるのに対し、NEP89は1.26×10⁻⁶ cm²/sで約1.00×10⁴倍高い。1500 Kでは約7.5×10⁻⁹に対して1.47×10⁻⁴ cm²/s、約1.96×10⁴倍である。NEP89直線の300 K外挿D=4.26×10⁻¹⁰ cm²/sはSethの1.08×10⁻¹¹ cm²/sの約39倍である。条件付きNernst–Einstein値0.103 mS/cmも引用実験0.0033 mS/cmの約31倍である。この伝導度はイオン相関を含まず、測定値として扱わない。[Marbellaら、2018；DOI: 10.1021/acs.chemmater.8b02812](https://doi.org/10.1021/acs.chemmater.8b02812)

![LiPON輸送とリン酸塩骨格運動](figures/24_LiPON_transport_framework.png)

骨格MSDによりArrhenius直線の過剰解釈を避ける。600 KではP/O/N運動はLiより小さい。1200 Kでは100 psの骨格最大MSDが8.91 Å²、1500 Kでは23.75 Å²となり、P単独でも18.36 Å²に達する。従って、高温2点はLi移動だけでなく網目再配置を含む。1500 Kではproduction前後1/4の間にポテンシャルエネルギーが14.36 meV/atom低下し、ほぼ定常な600 Kとは異なる。

![LiPON温度依存分波RDF](figures/25_LiPON_transport_RDF.png)

P–OとP–N第一殻は識別でき、平均幾何配位は1200 Kまでは大きく変わらない。一方、1500 KではP–N環境が前期から後期に変化し、最短N–N=1.225 Åの接触が再発する。Li–OとLi–Nピークは温度とともに広がり、Li移動増加と整合するが、高温網目変化のため、良好なArrhenius R²だけで4点全てを同一の固体輸送機構とは断定できない。

### 物理的解釈と結論

圧力と密度はセル平均であり、局所化学を保証しない。一方、短距離のみで特定反応も確定できない。作製比較と輸送系列を合わせると、NEP89はPreparation Bの低温リン酸塩骨格を保持するが、窒素トポロジーと高温網目運動は軌跡に敏感である。

**LiPONの結論：** Preparation B系列は単調な熱活性化Li運動を再現したが、文献の絶対値は再現しない。見かけのE_aは0.412 eVである一方、Dと条件付き伝導度を大幅に過大評価し、1500 K網目は定常な固体骨格ではない。従ってLiPONは汎用事前学習NEP89の明確な適用限界を示す。低温で妥当な開始構造を得ても、定量的輸送精度は保証されない。AとCは削除せず、作製移植性の失敗例として残す。

<a id="remaining"></a>
## 6. 総括：再現できた点・偏差・次の課題

結論は二つに分ける。実測した計算系ではNEPの計算コストが低く、採用の動機となった。一方、文献比較は一様な予測精度を示さない。LZOCはAIMDのトレーサー拡散を過小評価し、LSZCは温度傾向を再現せず、Li₃PS₄は局所構造の部分的一致に対して輸送の定量的一致を示さず、LiPONは輸送を過大評価し高温骨格運動を生じる。実行時間だけでなく、これらの材料依存の結果が現時点の適用限界を示す。LSZC端点NPTは完了したが、350 Kの末段密度低下が残り、両温度の平衡確認には至らない。

|材料・項目|現在の結果|追加計算|
|---|---|---|
|LSZC|完了済み320／350 K比較から有効なNEP E_aは得られない|320／330／340／350 Kの共通セルNVT50＋300 psを2系列投入。解析待ち。|
|LZOC|2 fs主図・表とAIMD D*直接比較を更新。|追加刻み幅比較なし。|
|Li₃PS₄|R1の作製・基本構造確認が完了。既存輸送比較を保持|R1だけで300／500／700／900 K輸送へ進む。新規輸送結果待ち。|
|LiPON|3組作製とBによる600／900／1200／1500 K輸送を解析。見かけのE_a=0.412 eVだがDを大幅に過大評価し、1500 Kで再構成|追加の盲目的productionは行わず、NEP89適用限界の結果として保持。|
...
|LiPON|124：Li47P16O56N5|3組作製とBによる600／900／1200／1500 K・各300 ps輸送、除圧、RDF、配位、時間刻み対照を解析|D(T)は単調だが絶対値を大幅に過大評価し1500 K骨格が再配置|
|旧LZOC|192、同公称LZOC|600 K詳細比較、700–900 K MSD／RDF、四温度計時を解析済み|計算効率・構造感度の比較であり精度順位ではない|

最新LSZC端点のproductionと解析は完了した。Li₃PS₄輸送、LiPONの3組作製とPreparation Bのバルク輸送は上記で解析済みである。DFTは投入していない。
