...
四温度すべてで硫酸根を維持し、中間温度だけに現れる局所構造の不連続は見られない。一方、Zrは全温度でEXAFS参照よりO配位が少なくCl配位が多い。直接カットオフ配位数とEXAFSフィット配位数は同一定義ではなく、部分RDFも実験全PDFではない。したがって、四温度RDFは異常な輸送傾向を単一の局所構造崩壊で説明しないが、作製法やポテンシャルの限界を考察する証拠となる。
