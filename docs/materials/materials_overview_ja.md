...
**2026年9月15日に追加の動力学反復を投入：** 配列ジョブ **8677221.1–4**。タスク1/2は340 K（速度シード34009151/34009152）、3/4は360 K（36009151/36009152）。各温度の元の192原子初期座標と固定セル、NEP89、時間刻み2 fs、NVT Nosé–Hoover鎖の結合時間50ステップ（100 fs）を維持する。既存速度を除いてシード付き初期化を行い、50 psのNVT平衡化後に300 psのproductionを別途保存する。この追加50 psは本研究の設定であり、文献で確認された指定値ではない。同一構造の速度シード反復であって、独立に作製したガラスではない。全4反復が完了し、同じ20–80 ps区間で解析済み。各productionは3000構造と6000行の有限熱力学値を含む。不利な結果もすべて示し、既存の元データは保存する。

### 完了した300 ps系列による文献比較

タスク8676684.1–3は340／360／380 Kで300 psを完了した。192原子、2 fs、NVT Nosé–Hoover鎖、結合周期100 fs。各再計算は元入力から開始し、旧80 ps軌跡は保存して連結しない。独立ガラスreplicaではない。

![LZOC輸送とAIMD比較](../../results/plots/amorphous/01_LZOC_transport.png)

a–cは各温度で傾き安定性に基づき選んだ1軌跡を、共通y軸・0–300 ps全ラグで表示し、フィット線・陰影は付けない。AIMD数値は選択に使用せず、正の傾きとR²≥0.99を条件に、解析窓を±10 ps移動した際の最大相対傾き変化が最小となるものを選んだ。候補窓は20–80、20–100、20–150、40–150、50–200 ps。これは解析後の表示用選択であり、replica全体の不偏推定や漸近的拡散の証明ではない。dは単独軌跡の見かけのDとHussain2024補足Table 4を比較し、文献誤差を維持する。反復平均・SDは描画しない。300 psでは時間原点ペアは一つのみ。

|T (K)|選択ジョブ|解析窓 (ps)|D_app (cm²/s)|MSD R²|近傍傾き変化|
|---:|---|---:|---:|---:|---:|
|340|8677221.1 (R1)|20–150|4.479×10⁻⁷|0.9978|1.62%|
|360|8676684.2 (元系列)|20–150|3.309×10⁻⁷|0.9962|2.04%|
|380|8676684.3 (元系列)|20–100|1.028×10⁻⁶|0.9994|1.94%|

選択値はAIMDの21.4%、18.7%、29.4%に留まり、温度に対して単調ではない。平衡化履歴・解析窓の差を明記し、この表示用の混合選択から新しい材料Eaを推定しない。以下の元系列・反復の固定窓表は監査記録として残し、上図の描画値とは区別する。

|T (K)|既存データのジョブ|AIMD D* (cm²/s)|NEP 300 ps D_app (cm²/s)|NEP/AIMD|追加反復|
|---:|---|---:|---:|---:|---|
|340|8676684.1|(2.09±0.06)×10⁻⁶|3.105×10⁻⁷|0.149|8677221.1–2：完了・解析済み|
|360|8676684.2|(1.77±0.04)×10⁻⁶|3.808×10⁻⁷|0.215|8677221.3–4：完了・解析済み|
|380|8676684.3|(3.50±0.10)×10⁻⁶|1.015×10⁻⁶|0.290|追加投入なし|

上記Dおよび現在の見かけのE_a = 0.3255 eV（Arrhenius R² = 0.8528）は既存系列のみの結果であり、追加反復を含む平均値ではない。追加反復の結果を以下に示す。追加平衡化で履歴が異なるため元系列と混合平均しない。380 Kの対応する新反復がないため、新たな三温度E_aは提示しない。

|追加反復|ジョブ|速度シード|NVT平衡化 (ps)|Production (ps)|時間刻み (fs)|D_app (cm²/s)|
|---|---|---:|---:|---:|---:|---|
...
|T (K)|追加反復の平均±標本SD (cm²/s)、n=2|平均/AIMD|MSDフィットR² (R1/R2)|
|---:|---:|---:|---:|
|340|(5.669 ± 2.130)×10⁻⁷|0.271|0.9961 / 0.9978|
|360|(1.426 ± 0.897)×10⁻⁷|0.081|0.9842 / 0.8597|

**反復の結論：** 新しい360 K平均は340 K平均より低く、速度シードを増やしても安定した単調な温度依存性は確認できなかった。両平均ともAIMDより低い。360 K R2の線形性は弱く（R²=0.8597）、近傍区間の傾き変化は51.7%。360 K R1は14.6%、340 Kは6.1–6.4%。有利な軌跡を選ばず、有限区間の見かけの推定値として保持し、拡散収束を主張しない。production平均温度は340.30/339.94および359.85/360.25 K。最後−最初50 psの勢エネルギー変化はそれぞれ−4.23/−5.55、+0.54/−1.87 meV/atom。温度は目標付近だが、エネルギー緩和と反復差は制限として残る。以下のRDF・配位数・元素別運動解析は元ジョブ8676684に対応し、追加反復の構造解析ではない。

全温度でNEPはAIMDより低い。元のNEP系列は単調増加するが、参照値には340→360 Kの低下がある。逐点一致も厳密な温度傾向も再現していない。セルサイズ、作製履歴、ポテンシャルが異なるため、偏差の原因をポテンシャルだけに帰属させない。

### サンプリング延長で結果は安定したか

**元系列の主解析区間：20–80 ps。** 曲線確認後の選択であり、事前登録ではない。初期の急上昇を避け、近傍20–60／30–90 psとの傾きの差は全温度で4%未満。より後半では傾きが変化する。局所安定性に基づく実用的選択であり、唯一の最適区間や漸近的拡散を証明しない。現在の見かけの **E_aは0.3255 eV、Arrhenius R²=0.8528**。MSD R²は0.9953／0.9980／0.9989、log–log指数は0.524／0.490／0.647。文献Eaへの近さで選択していない。以下の10–40 ps結果は時間長感度の診断として保持し、0.428 eVは現在の主推定値ではない。

以下はすべて同じ10–40 psラグで解析し、軌跡全長のみを変更した。

|累積時間 (ps)|D340 (cm²/s)|D360 (cm²/s)|D380 (cm²/s)|診断用 E_a (eV)|Arrhenius R²|
|---:|---:|---:|---:|---:|---:|
...
これらは新計算の累積区間であり、旧80 psデータではない。同じ初期入力を保持しても新計算の最初80 psの推定値は旧計算と異なる。軌跡分岐の原因は特定しておらず、両者を混同しない。累積区間と連続ブロックは相関があり、独立replicaではない。

時間長への依存が大きく、300 psで拡散・活性化エネルギーの収束を確認できたとはいえない。300 ps系列の10–40 psにおけるlog–log MSD指数は0.282／0.316／0.566。局所運動のオフセットも影響するため、漸近的な異常拡散を証明する指標とはしない。0.428 eVは診断用フィットで、確定した材料E_aではない。新たな室温伝導率外挿は主結果とせず、従来の0.280 eVは旧80 ps解析に限る。

**解釈：** サンプリング延長は単に文献一致を改善するのではなく推定値を大きく変える。現段階の結論は見かけの輸送の限定的比較であり、AIMDの完全再現ではない。
