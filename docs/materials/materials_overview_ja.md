...
a–cは全時間原点平均・全系重心補正Li MSDの0–300 ps全ラグを表示し、網掛けと破線は20–80 psフィットを示す。dはその傾きと[Hussain2024](https://doi.org/10.1038/s41524-024-01346-y)補足Table 4を比較する。文献の±は原報のまま示し、2 fs系列のみ掲載する。300 ps終点の時間原点ペアは一つだけであり、末尾を短ラグと同程度に十分な統計として扱わない。
...
#### 300 ps軌跡全体に対する広い区間の比較

|ラグ区間 (ps)|D340 (×10⁻⁷ cm²/s)|D360 (×10⁻⁷ cm²/s)|D380 (×10⁻⁷ cm²/s)|
|---|---:|---:|---:|
|20–80、現在の主解析|3.105|3.808|10.155|
...
20–150 psでは20–80 psに比べDが約23／13／15%、50–200 psでは26／23／27%低下する。この系統的な区間依存は共通の傾きのプラトーを示さない。長い区間は感度解析として保持し、長いという理由だけで収束が改善したとは扱わない。現在の主解析20–80 psは暫定であり、唯一の最適区間ではない。時間原点平均には300 ps全体が寄与する。本比較では追加計算を投入していない。
