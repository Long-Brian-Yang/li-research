...
灰色は元系列、青・赤は340/360 Kの追加速度反復。dには各反復と算術平均±標本SD（n=2、信頼区間ではない）を示し、元系列は分離する。a–cは全時間原点平均・全系重心補正Li MSDの0–300 ps全ラグを表示し、見やすさのためフィットの重ね描きは省略する。dは20–80 psの傾きと[Hussain2024](https://doi.org/10.1038/s41524-024-01346-y)補足Table 4を比較する。文献の±は原報のまま示し、2 fs系列のみ掲載する。300 ps終点の時間原点ペアは一つだけであり、末尾を短ラグと同程度に十分な統計として扱わない。
