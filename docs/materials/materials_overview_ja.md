...
結論は二つに分ける。実測した計算系ではNEPの計算コストが低く、採用の動機となった。一方、文献比較は一様な予測精度を示さない。新LZOCはAIMDのトレーサー拡散を過小評価し、LSZCは温度傾向を再現せず、Li₃PS₄は局所構造の部分的一致に対して輸送の定量的一致を示さず、LiPONには局所接触の問題が残る。実行時間だけでなく、これらの材料依存の結果が現時点の適用限界を示す。提案済みのLSZC平衡化・長時間サンプリングは計画であり、投入済み・完了済みの計算ではない。
