...
全4温度の完全MSDはlag timeとともに増加した。600 Kの主軌跡は20–150 psでD=1.57×10⁻⁶ cm²/sとなった。600 Kは20–150 ps、他温度は20–100 psを用いた。いずれもtime-origin平均であり、図上にフィット線は重ねない。主区間のR²=0.9997–0.9999、log–log指数α=0.92–1.03で、ほぼ拡散的である。選択は文献値への近さではなく、広い線形区間と隣接窓に対する傾きの安定性に基づく。
...
**LiPONの結論：** 600 Kの600 ps軌跡と600–1500 KのNEP89系列は、単調な熱活性化Li運動を示す。4点から得た**E_a=0.389 eV**は実験基準より低く、共通温度のDと300 K外挿値は文献より大きい。したがって、NEP89は定性的な温度依存性を再現するが、このLiPONモデルの定量的輸送予測には系統的な過大評価が残る。
...
|LiPON|3組作製とBによる600／900／1200／1500 K輸送を解析。600 Kは600 psの1軌跡を採用し、E_a=0.389 eV；Dは文献より大きい|追加MDは不要。材料専用ポテンシャルとの差を適用限界として扱う。|
...
|LiPON|124：Li47P16O56N5|3組作製、Bによる600 K・600 psの1軌跡および900／1200／1500 K・300 ps輸送、除圧、RDF、配位を解析|D(T)は単調、E_a=0.389 eV；絶対値は文献より大きい|
