...
|LSZC|272；Li32Zr32Cl128S16O64|独立クラスター配置・緩和、400 K保持、20 ps NPT、RDF／配位／EXAFS／截断・硫酸根接続性比較；400 K輸送試行を投入|硫酸根は保持。密度とZr環境に差が残る|
|Li₃PS₄|512；Li192P64S256|作製と4温度200 ps本計算を解析済み；MSD、D、NE換算、ブロック、RDF／接続性、文献比較|見かけのDが参照値より大きい；300 Kプラトーと900 K骨格変化が制約|
|LiPON|124；Li47P16O56N5|作製、250 K減圧、N–N接触追跡、配位、0.5／0.25 fs対照を解析|短い接触が持続。検証済み輸送構造ではない|
...
先行診断では座標・セル・速度を保持し、NEPハッシュを検査した。先行診断の熱力学量は0.05 ps、軌跡は0.1 ps間隔。追加のLiPON・LSZC計算の出力間隔は下表の条件を用いる。NPTは1 bar＝0.0001 GPa、温度／圧力周期100／1000 fs。DFT、人為的な密度調整、目標Eaに合わせた軌跡選択は含めない。

<a id="followup"></a>

### 追加計算条件と投入履歴

2026年9月15日の最終記録では、**LSZC 8676040.3は80／200 psまで実行中**だった。今回の文書編集では、その後の完了を確認していない。LiPON8676040.1／2は新しい遠隔入力ディレクトリの同期前にハッシュ検証で終了し、MDは実行されなかった。ディレクトリを作成し入力ハッシュを確認後、**LiPONだけを8676059.1／2として再投入し、両方2 ps完了・解析済み**である。
...
標準偏差は同一軌跡内の変動で、標準誤差・信頼区間・独立構造間の不確かさではない。380 Kのブロック値は1.865×10⁻⁶から約0.950×10⁻⁶ cm²/sへ低下した。高いR²だけで拡散収束とは判定できない。有限切片の影響もあるため、両対数勾配が1未満という理由だけで異常拡散とも断定しない。検証済みEa・室温予測とはしない。NHC系列の条件付き外挿は後節に別記する。データ：`LZOC_*K_msd.csv`、`LZOC_*K_block_D.csv`、`summary.json`。
...
10–40 psのE_aはHussainの0.25±0.10 eVより0.0303 eV大きい。σ300=4.74 mS/cmは理論43.3±3.3 mS/cmの0.110倍、Huの実験2.42 mS/cm（**298.15 K**）の1.96倍である。実験温度は厳密には300 Kではなく、試料状態も異なる。条件付き数値比較であり、検証済み室温予測ではない。全区間を残し、実験に近い区間を選別しない。[数値表](../../results/amorphous_review_20260915/portfolio_supplement/LZOC_300K_conditional.csv)、[計算スクリプト](../../scripts/structures/portfolio_supplement.py)。この表は診断的外挿であり、検証済み外挿として採用しない。
...
**参照単位は図の軸から確認済み：** [公式Fig.3a](https://www.nature.com/articles/s41467-025-56322-x/figures/3)の縦軸はln[D(cm²/s)]で、本文と一致する。Excelの電導率(S/cm)という列名は図と矛盾する。元の値・見出しは変更せず、派生比較では図の軸を根拠にDとして扱うことを明記した。これは著者のDeePMDガラス結果であり、実験Dや本研究のAIMDではない。他材料の制約は引き続き残る。

<a id="lps-transport"></a>
### 4温度輸送：対照解析完了、定量的には不一致

全出力をローカルとTSUBAMEに保存した。各本計算は0.1 ps間隔の2,000フレームと初期構造、0.05 ps間隔の4,000熱力学レコードからなる。512原子と元素順序を確認し、固定セルでminimum-image unwrap後に全系の質量重み付き重心移動を除去した。Liのみの重心は差し引いていない。[analysis.json](../../results/amorphous_review_20260915/Li3PS4_transport/analysis.json)にハッシュと数値検証を保存した。各温度1つのガラスであり、独立構造replicaではない。

![Li3PS4四温度Li MSD](../../results/amorphous_review_20260915/Li3PS4_transport/Li_MSD.png)

時間原点平均MSDであり、手動平滑化はしていない。200 ps全軌跡から計算し、図にはlag0–100 ps、CSVには全lagを保存した。300 Kのプラトーを見せるため**パネル間の縦軸範囲は異なる**。破線は全温度共通20–80 psで切片を自由にした直線回帰である。300 KではMSD(80 ps)=0.414 Å²、log–log傾き0.049で、長時間拡散は確立していない。500 Kのα<1には正の切片も寄与するため、指数だけで異常拡散とは判定しない。700／900 Kはこの時間範囲でより直線的である。

|T (K)|見かけのD、20–80 ps (cm²/s)|R²|Log–log傾きα|条件付きσ_NE (mS/cm)|
|---:|---:|---:|---:|---:|
...
電導率欄は**回帰傾きの条件付きNernst–Einstein換算**であり、実測値や検証済み電導率ではない。特に300 Kを収束した室温予測として扱わない。イオン間相関は無視している。τをps、MSDをÅ²とすると：
...
後式ではVとDをSI単位にし、S/mを10倍するとmS/cmになる。N_Li=192、Vは各固定セル体積、eは電気素量、k_BはBoltzmann定数、Tは設定温度である。[輸送数値表](../../results/amorphous_review_20260915/Li3PS4_transport/transport_summary.csv)。

![Li3PS4回帰区間とブロック依存性](../../results/amorphous_review_20260915/Li3PS4_transport/fit_sensitivity.png)

左は共通の5–20／10–40／20–80／40–100 ps回帰、右は互いに重ならない50 psブロック4つをそれぞれlag5–20 psで回帰した結果。対数縦軸で低温の感度も示す。300 KのDは区間により7.14e−9–4.99e−8 cm²/s。300／500／700／900 Kのブロック標準偏差は1.49e−8／8.01e−8／9.08e−7／3.68e−6 cm²/sである。単一ガラスの時間ブロックのばらつきであり、区間も異なるため**20–80 ps値の信頼区間や誤差棒ではない**。[全回帰結果](../../results/amorphous_review_20260915/Li3PS4_transport/fit_windows.csv)。

![Li3PS4文献輸送比較](../../results/amorphous_review_20260915/Li3PS4_transport/D_reference_comparison.png)

Chen Fig.3aのガラス数値系列を元のまま用い、公式縦軸を確認してDと解釈した。点間の線はガイドであり、全温度Arrhenius回帰ではない。温度上昇に伴う移動度増加は共通するが、**定量的には一致しない**。ポテンシャル、作製過程、密度、時間長、拡散領域の違いがあり、差をポテンシャルだけに帰属できない。[比較CSV](../../results/amorphous_review_20260915/Li3PS4_transport/reference_comparison.csv)。

|T (K)|ChenガラスD (cm²/s)|NEPの見かけのD／ChenのD|
|---:|---:|---:|
|300|1.186e−9|6.02|
|500|9.350e−8|7.58|
|700|1.524e−6|5.30|
...
**500／700／900 Kだけ**でlnD対1/Tを診断的に回帰すると、見かけのE_a=0.3795 eV、R²=0.99927となる。論文のガラス注記は0.47 eVだが、回帰範囲と構造条件が一致しておらず、制御された精度評価ではない。この回帰を300 Kまで外挿せず、検証済みのガラス障壁として採用しない。

![Li3PS4骨格運動](../../results/amorphous_review_20260915/Li3PS4_transport/framework_MSD.png)

Liの大きなスケールに隠れないようP／Sだけを表示した。80 psでP／SのMSDは300 Kの0.0807／0.137 Å²から900 Kの5.99／12.50 Å²まで増加する。高温骨格は不動ではなく、変化しない骨格中のLi跳躍だけで輸送を説明できない。運動とRDFだけで融解とも断定しない。

![Li3PS4前後半RDF](../../results/amorphous_review_20260915/Li3PS4_transport/RDF_early_late.png)

前半0.1–45.1 ps、後半150.1–195.1 psから各10フレームを抽出した。bin幅0.05 Å、平滑化なし、同じ元素対の縦軸は共通である。P–S第一殻は残るが、RDFピークが似ていても接続性不変とは限らない。P–S<2.6 Å、P–P<2.6 Åの定義で、300／500／700／900 Kの後半P四配位率は100／100／99.69／94.38%、孤立した幾何学的P1S4に属するPの割合は79.69／79.69／83.13／80.47%。900 Kの四配位率は前半99.38%から後半94.38%へ低下する。幾何学成分に化学的電荷状態は付与していない。[フレーム別成分](../../results/amorphous_review_20260915/Li3PS4_transport/geometric_components.csv)。

![Li3PS4熱力学診断](../../results/amorphous_review_20260915/Li3PS4_transport/thermodynamics.png)

読みやすさのため5 ps平均を表示し、全解像度CSVも保存した。本計算平均温度は300.32／500.65／700.32／899.88 K、固定セル密度は2.2270／2.1493／2.0915／1.9886 g/cm³。**NVTで密度が一定なのは拘束条件であり、密度収束の証拠ではない**。平均圧力は+0.252／−0.120／−0.134／−0.244 GPaであり、最後の瞬間的NPTセルがNVTで平均圧力平衡になるとは限らない。900 KのNPT最初／最後の四分の一期の平均密度は2.0314→2.0155 g/cm³と変化しており、残留緩和と後続の接続性変化を制約として残す。

**結論：** 単一ガラスの4温度後処理と文献比較は完了した。局所構造の類似はあるが輸送は参照値より速く、低温・高温の制約が残る。新規計算は追加していない。再現成功ではなく、体積・作製条件を揃えた対照や低温長時間化は別の継続課題である。現結果を都合よく変更しない。
...
|新LZOCのD／Ea／σ300|区間・ブロック感度と条件付き外挿を掲載、未収束|継続する場合は観測時間と独立標本の確認を事前に定める。好ましいEaを選ばない|
|LSZC構造・輸送|NPT・截断・接続性解析済み。400 K試行を投入、最終記録80／200 ps|完了出力を確認後、MSD／D・ブロック・骨格運動を解析。構造差を解釈に残す|
|Li₃PS₄輸送と種の割合|8675738完了・解析済み；参照D単位確認；6図群と数値表を追加|定量再現には未到達；300 Kプラトー、900 K骨格変化、作製条件差が残る；グラフから化学種割合は断定しない|
|LiPONの化学状態|形成過程と時間刻み対照を解析済み。両分岐99%のサンプルで短い接触が持続|限界として保存。単なるMD延長で化学妥当性を保証しない。DFTは利用・依頼なし|
...
LSZC・LiPONの追加診断を含む既存の図を、説明とともに材料別に集約した。文書の網羅性を整えたのであって、全材料の定量的な実験一致が完了したわけではない。旧報告とGit履歴に以前の状態を残し、最新情報は本書のみで管理する。
