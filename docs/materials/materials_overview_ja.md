...
**2026年9月15日に追加の動力学反復を投入：** 配列ジョブ **8677221.1–4**。タスク1/2は340 K（速度シード34009151/34009152）、3/4は360 K（36009151/36009152）。各温度の元の192原子初期座標と固定セル、NEP89、時間刻み2 fs、NVT Nosé–Hoover鎖の結合時間50ステップ（100 fs）を維持する。既存速度を除いてシード付き初期化を行い、50 psのNVT平衡化後に300 psのproductionを別途保存する。この追加50 psは本研究の設定であり、文献で確認された指定値ではない。同一構造の速度シード反復であって、独立に作製したガラスではない。不利な結果も含め、全反復を同じ方法で解析・保存し、既存結果は上書きしない。投入確認済みで、完了・解析は未確認である。
...
a–cは全時間原点平均・全系重心補正Li MSDの0–300 ps全ラグを表示し、見やすさのためフィットの重ね描きは省略する。dは20–80 psの傾きと[Hussain2024](https://doi.org/10.1038/s41524-024-01346-y)補足Table 4を比較する。文献の±は原報のまま示し、2 fs系列のみ掲載する。300 ps終点の時間原点ペアは一つだけであり、末尾を短ラグと同程度に十分な統計として扱わない。

|T (K)|既存データのジョブ|AIMD D* (cm²/s)|NEP 300 ps D_app (cm²/s)|NEP/AIMD|追加反復|
|---:|---|---:|---:|---:|---|
|340|8676684.1|(2.09±0.06)×10⁻⁶|3.105×10⁻⁷|0.149|8677221.1–2：投入済み、解析待ち|
|360|8676684.2|(1.77±0.04)×10⁻⁶|3.808×10⁻⁷|0.215|8677221.3–4：投入済み、解析待ち|
|380|8676684.3|(3.50±0.10)×10⁻⁶|1.015×10⁻⁶|0.290|追加投入なし|

上記Dおよび現在の見かけのE_a = 0.3255 eV（Arrhenius R² = 0.8528）は既存系列のみの結果であり、追加反復を含む平均値ではない。追加反復のDとばらつきは解析待ちであり、投入だけから新しい数値結果を推定しない。

|追加反復|ジョブ|速度シード|NVT平衡化 (ps)|Production (ps)|時間刻み (fs)|D_app (cm²/s)|
|---|---|---:|---:|---:|---:|---|
|340 K R1|8677221.1|34009151|50|300|2|解析待ち|
|340 K R2|8677221.2|34009152|50|300|2|解析待ち|
|360 K R1|8677221.3|36009151|50|300|2|解析待ち|
|360 K R2|8677221.4|36009152|50|300|2|解析待ち|

全温度でNEPはAIMDより低い。NEP系列は単調増加するが、参照値には340→360 Kの低下がある。逐点一致も厳密な温度傾向も再現していない。セルサイズ、作製履歴、ポテンシャルが異なるため、偏差の原因をポテンシャルだけに帰属させない。

### サンプリング延長で結果は安定したか

**現在の主解析区間：20–80 ps。** 曲線確認後の選択であり、事前登録ではない。初期の急上昇を避け、近傍20–60／30–90 psとの傾きの差は全温度で4%未満。より後半では傾きが変化する。局所安定性に基づく実用的選択であり、唯一の最適区間や漸近的拡散を証明しない。現在の見かけの **E_aは0.3255 eV、Arrhenius R²=0.8528**。MSD R²は0.9953／0.9980／0.9989、log–log指数は0.524／0.490／0.647。文献Eaへの近さで選択していない。以下の10–40 ps結果は時間長感度の診断として保持し、0.428 eVは現在の主推定値ではない。
