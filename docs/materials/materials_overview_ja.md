...
![材料横断の輸送文献対応](figures/26_cross_material_literature_correspondence.png)

パネルaは温度ラベルごとに、対応する物理量の$\log_{10}(\mathrm{NEP89}/\mathrm{文献})$を示す。LZOC、Li₃PS₄、LiPONはトレーサー拡散、LSZCは条件付き伝導度と調整済みMACE伝導度の比較である。ゼロは数値一致、負は過小評価、正は過大評価を意味する。パネルbは文献値と多温度NEP89フィットをともに持つ材料の活性化エネルギーを比較する。LSZCは実験一致線に近く、Li₃PS₄は絶対拡散が大きくても温度感度は近い。LiPONは活性化エネルギーが低く、拡散絶対値も大きい。したがって、転移性は化学系に依存し、絶対輸送と温度感度を分けて評価する必要がある。
...
比の範囲はパネルaに示す同一物理量・共通温度だけから求め、異なる輸送定義を平均していない。この表を局所構造節と合わせて読むことで、特徴的構造、温度感度、絶対輸送の三種類の一致を区別できる。
...
この結果は実用的なモデル運用方針も与える。汎用NEP89は、迅速なガラス構築、長時間機構探索、相対的温度傾向の評価に適する。定量伝導度は証拠レベルに応じて扱い、LSZCは現状の実験対応解析を発展させ、Li₃PS₄はホッピング環境解析を継続し、LiPONは絶対輸送を予測へ用いる前に材料専用学習または校正を優先する。これにより、モデル差を単なる失敗ではなく、次の計算手法を選ぶ判断基準へ変換できる。
