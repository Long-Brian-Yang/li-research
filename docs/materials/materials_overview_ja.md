...
図は本研究の4温度系列を結び、Sethらが本文で数値を示したmelt-quench構造の600 Kおよび1500 Kを同じ単位で重ねる。NEP89は温度上昇に伴う拡散増加を示す一方、両方の共通温度で材料専用NequIPより約4桁高い。従って、本比較で再現されたのは熱活性化傾向であり、拡散係数の絶対値ではない。
