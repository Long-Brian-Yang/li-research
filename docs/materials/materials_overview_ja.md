...
Preparation Bは拡散係数を見る前に選定した。各文献バルク温度で250 K構造を10 ps NPT昇温し、1 barで50 ps NPT平衡化、その後固定セルNVTで300 ps productionを実行した。時間刻みは0.5 fs、構造出力は0.1 psごとであり、各軌跡は3000 production frameを含む。温度点はSethに対応するが、平衡化／production長、MTTK結合、NEP89は本研究の設定である。

![LiPONバルクLi MSD](figures/23_LiPON_bulk_MSD.png)

全4温度の完全MSDはlag timeとともに増加した。主比較には共通の20–100 ps time-origin平均フィットを用いる。範囲は本節で一度だけ明記し、図上には重ねない。各温度でR²=0.9973–0.9999、log–log指数α=0.87–1.08であり、この区間ではほぼ拡散的である。広い範囲を含む感度結果はsource dataに保持し、文献への近さでは選んでいない。
...
4点のNEP89拡散係数から得たArrhenius活性化エネルギーは**0.409 eV**（R²=0.9962）であり、LiPON薄膜で一般に報告される実験値約0.55 eVより低い。600 KでSethのmelt-quench LiPONは1.25×10⁻¹⁰ cm²/sであるのに対し、NEP89は1.26×10⁻⁶ cm²/sで約1.00×10⁴倍高い。1500 Kでは約7.5×10⁻⁹に対して1.43×10⁻⁴ cm²/sで、約1.91×10⁴倍高い。NEP89直線の300 K外挿D=5.00×10⁻¹⁰ cm²/sはSethの1.08×10⁻¹¹ cm²/sの約46倍である。対応する条件付きNernst–Einstein値は0.120 mS/cmで、引用実験値0.0033 mS/cmの約36倍である。この伝導度はイオン相関を含まない換算値であり、測定値ではない。[Sethら、2025；DOI: 10.1021/acsmaterialsau.4c00117](https://doi.org/10.1021/acsmaterialsau.4c00117) [Marbellaら、2018；DOI: 10.1021/acs.chemmater.8b02812](https://doi.org/10.1021/acs.chemmater.8b02812)

![LiPONのArrhenius比較](figures/24_LiPON_Arrhenius.png)

左図は本研究の4温度系列を結び、Sethらが本文で数値を示したmelt-quench構造の600 Kおよび1500 Kを同じ単位で重ねる。NEP89は温度上昇に伴う拡散増加を示す一方、両方の共通温度で材料専用NequIPより約4桁高い。右図は物理量を混同しないよう別軸とし、300 K外挿Dから得た条件付きNernst–Einstein伝導度0.120 mS/cmを実験値0.0033 mS/cmと比較する。従って、本比較で再現されたのは熱活性化傾向であり、拡散係数または伝導度の絶対値ではない。
...
**LiPONの結論：** 600–1500 KのNEP89系列は単調な熱活性化Li運動を示し、4点から**E_a=0.409 eV**を得た。一方、共通温度のDと300 K外挿値は文献より大きい。したがって、NEP89は定性的な温度依存性を再現するが、このLiPONモデルの定量的輸送予測には系統的な過大評価が残る。
...
|LiPON|3組作製とBによる600／900／1200／1500 K輸送を解析。E_a=0.409 eVで、Dは文献より大きい|低温側の確認として600 K・600 psを2組実行中。完了後に現行600 K値と比較する。|
...
|LiPON|124：Li47P16O56N5|3組作製とBによる600／900／1200／1500 K・各300 ps輸送、除圧、RDF、配位、時間刻み対照を解析|D(T)は単調、E_a=0.409 eV；絶対値は文献より大きい|
