...
本書は実用的な事前学習ポテンシャル計算系のベンチマークであり、新しいポテンシャルの学習研究でも、引用した全シミュレーションの厳密再現でもない。最初にMACEとNEPを比較して計算手法の選択理由を示し、その後の材料節でこの選択を文献に対して検証する。初期構造、セルサイズ、系統、温度制御、軌跡長の差は、解釈に影響する箇所で明記する。偏差は目標値へ近づけるよう軌跡を調整して消すのではなく、結果として保持する。

従って、本書は方法選択から次第に厳しい材料検証へ進む。すなわち、MACE対NEPの効率比較、LZOCのAIMD比較、LSZCの実験比較への展開、Li₃PS₄による硫化物への転移性、最後にLiPONによる適用限界の順である。
