#!/usr/bin/env bash
# Canonical MACE-MP-0b3-medium ML-IAP/Kokkos CUDA benchmark.
#$ -cwd
#$ -V
#$ -l gpu_1=1
#$ -l h_rt=0:30:00
...
MODEL="$MODELS_ROOT/mace/mace-mp-0b3-medium.model-mliap_lammps.pt"
DATA="$STRUCTURES_ROOT/Li3YCl6_ordered_03_2x2x2.data"
OUT="$BENCHMARKS_ROOT/lammps/mace_mliap_gpu/mace_mp0b3_medium_canonical"
module purge
...
mkdir -p "$OUT"
printf 'model=MACE-MP-0b3-medium\npython=%s\n' "$($PY -V 2>&1)" > "$OUT/preflight.txt"
